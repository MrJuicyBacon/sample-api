...
class UsersOrdersHandler(SampleHandler):
    def __init__(self, books_as_id=True, shops_as_id=True, *args, **kwargs):
        ...
        self.handler = lambda x: self._handler_function(x, books_as_id, shops_as_id)

    async def _handler_function(self, request, books_as_id=True, shops_as_id=True):
        session = DbSession
        user_id = request.match_info.get('user_id')
        ...
        books = self._get_objects_from_ids(Book, book_ids) if not books_as_id and len(book_ids) else None
        shops = self._get_objects_from_ids(Shop, shop_ids) if not shops_as_id and len(shop_ids) else None
